...
public class Main {
	...
	public Main() {
		orderManager = OrderManager.getInstance();		//Đơn hàng duy nhất
		invoiceManager = InvoiceManager.getInstance();  //Hóa đơn duy nhất
		menu = new ArrayList<>();
		...
		// Khởi tạo các dữ liệu ban đầu (thực đơn, khuyến mãi, nguyên liệu, nhà cung cấp)
		initializeMenu();
	...
	private void addNewDish(Scanner scanner) {
		...
		DishFactory factory;
		if (typeChoice == 1) {
			factory = new MainCourseFactory();		//Món chính
		} else if (typeChoice == 2) {
			factory = new DessertFactory();			//Món tráng miệng
		} else {
